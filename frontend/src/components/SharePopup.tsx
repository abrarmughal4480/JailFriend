...
const SharePopup: React.FC<SharePopupProps> = ({
  ...
  const handleSocialShare = (platform: string) => {
    const text = shareOptions.customMessage || getCleanContent(postContent) || (isAlbum ? 'Check out this album!' : 'Check out this post!');
  ...
  const getCleanContent = (content: string) => {
    if (!content) return '';
    // First extract content from pre tags if they exist
    const preMatch = content.match(/<pre[^>]*>([\s\S]*?)<\/pre>/);
    const text = preMatch ? preMatch[1] : content;
          ...
          <div className="p-4 sm:p-6">
                ...
                <div className="flex items-start gap-3 sm:gap-4">
                  {postMedia && postMedia.length > 0 && (
                    <div className="relative w-16 h-16 sm:w-20 sm:h-20 bg-gray-200 dark:bg-gray-600 rounded-lg overflow-hidden flex-shrink-0">
                      {(() => {
                        ...
                        const url = typeof media === 'string' ? media : (media?.url || media?.secure_url || '');
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm text-gray-700 dark:text-gray-300 line-clamp-3 leading-relaxed">
                      {getCleanContent(postContent) || (isAlbum ? 'Album' : 'Post')}
                    </p>
                    {isAlbum && postMedia && (
            ...
            <div className="flex bg-gray-100 dark:bg-gray-700 rounded-xl p-1 mb-4 sm:mb-6">
              <button
                ...
                className={`flex-1 py-2 sm:py-3 px-3 sm:px-4 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 touch-manipulation ${shareMode === 'internal'
              ...
              <button
                ...
                className={`flex-1 py-2 sm:py-3 px-3 sm:px-4 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 touch-manipulation ${shareMode === 'social'
                  ? 'bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm'
                  : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'
                  }`}
                style={{ touchAction: 'manipulation' }}
                ...
                <div className="space-y-4 mb-6">
                  <label className="flex items-center space-x-3 cursor-pointer p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <span className="text-gray-700 dark:text-gray-300 font-medium">Share on my Feed</span>
                        <p className="text-xs text-gray-500 dark:text-gray-400">Post to your profile Feed</p>
                      </div>
                    </div>
                  </label>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    {socialPlatforms.map((platform) => (
                      <button
                        ...
                        className={`flex items-center space-x-2 sm:space-x-3 p-3 sm:p-4 rounded-xl border-2 border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-500 transition-all duration-200 hover:shadow-md touch-manipulation ${shareOptions.socialPlatforms?.includes(platform.id)
