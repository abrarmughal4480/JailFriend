...
const AutoPageLike = () => {
  ...
  const handleSave = async () => {
    if (!pageNames.trim()) {
      alert('Please enter at least one page name');
      return;
    }

    setSaving(true);
    // Add API call here
    console.log('Saving auto page like page names:', pageNames);
    
    // Simulate API call
    setTimeout(() => {
      setSaving(false);
      alert('Auto page like settings saved successfully!');
    }, 1000);
  };
    ...
    <div className={`p-6 min-h-screen transition-colors duration-200 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="mb-6">
        ...
        <div className={`text-sm transition-colors duration-200 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          Tools
          {' > '}
          <span className="text-red-500 font-semibold">Auto Page Like</span>
        </div>
      </div>
      ...
      <div className={`rounded-lg shadow-md border transition-colors duration-200 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        <div className="p-6">
          ...
          <div className={`mb-6 p-4 rounded-lg border transition-colors duration-200 ${
            ...
            <p className="text-sm">
              When a user creates a new account, choose which page you would like to get auto liked.
            </p>
          </div>
          ...
          <div className="mb-6">
            ...
            >
              Page name(s), sperated by a comma (,)
            </label>
            <input
              type="text"
              id="pageNames"
          ...
          <div className="pt-4">
            <button
              onClick={handleSave}
              disabled={saving || !pageNames.trim()}
