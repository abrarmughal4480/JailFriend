...
const ManageProducts = () => {
  ...
  const getAuthorName = (product: Product) => {
    if (product.seller?.name) return product.seller.name;
    if (product.seller?.username) return product.seller.username;
    return 'Unknown';
  };

  const paginatedProducts = searchQuery.trim() 
    ? filteredProducts 
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex mb-4" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-2 text-sm">
            ...
            <li>
              <span className={`${isDarkMode ? 'text-red-400' : 'text-red-500'} font-medium`}>Manage Products</span>
            </li>
          </ol>
        </nav>
                  ...
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center">
                      <div className="flex items-center justify-center">
                        ...
                        <span className={`ml-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading products...</span>
                      </div>
                    </td>
                  </tr>
                    ...
                    <tr key={product._id} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                      ...
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {product.link ? (
                          ...
                          >
                            {product.link.length > 30 ? product.link.substring(0, 30) + '...' : product.link}
                          </a>
                        ) : (
