...
const Jobs = () => {
  ...
  const handleSelectJob = (jobId: string, checked: boolean) => {
    if (checked) {
      ...
      setSelectedJobs(prev => prev.filter((id: string) => id !== jobId));
    }
  };
  ...
  const handleDeleteSelected = () => {
    if (selectedJobs.length > 0) {
      if (confirm(`Are you sure you want to delete ${selectedJobs.length} selected job(s)?`)) {
        setJobs(prev => prev.filter((job: Job) => !selectedJobs.includes(job.id)));
        setSelectedJobs([]);
  ...
  const isAllSelected = selectedJobs.length === totalJobs && totalJobs > 0;
              ...
              <table className="w-full min-w-[600px]">
                ...
                <tbody className={`${isDarkMode ? 'bg-gray-800 divide-gray-700' : 'bg-white divide-gray-200'} divide-y`}>
                  {jobs.length === 0 ? (
                    ...
                    jobs.map((job) => (
                      <tr key={job.id} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                        ...
                        <td className={`px-3 py-2 text-xs ${isDarkMode ? 'text-gray-300' : 'text-gray-900'} font-medium`}>{job.id}</td>
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-2">
                            <img
                              src={job.publisherAvatar || '/default-avatar.svg'}
                              alt={job.publisher}
                            ...
                            />
                            <span className={`text-xs ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{job.publisher}</span>
                          </div>
                        </td>
                        <td className={`px-3 py-2 text-xs ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{job.title}</td>
                        <td className="px-3 py-2">
                          <a
                            href={job.jobLink}
                            target="_blank"
                        ...
                        <td className={`px-3 py-2 text-xs ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{job.posted}</td>
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-2">
                            <button
