...
const Offers = () => {
  ...
  const [offers, setOffers] = useState<Offer[]>([
    ...
    //   offerLink: 'https://example.com/offer/1',
    //   posted: '2 hours ago'
    // }
  ]);
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedOffers(offers.map((offer: Offer) => offer.id));
    } else {
      setSelectedOffers([]);
    }
  };
  ...
  const handleSelectOffer = (offerId: string, checked: boolean) => {
    if (checked) {
      ...
      setSelectedOffers(prev => prev.filter((id: string) => id !== offerId));
    }
  };

  const handleDeleteSelected = () => {
    if (selectedOffers.length > 0) {
      if (confirm(`Are you sure you want to delete ${selectedOffers.length} selected offer(s)?`)) {
        setOffers(prev => prev.filter((offer: Offer) => !selectedOffers.includes(offer.id)));
        setSelectedOffers([]);
  ...
  const handleDeleteOffer = (offerId: string) => {
    if (confirm('Are you sure you want to delete this offer?')) {
      setOffers(prev => prev.filter((offer: Offer) => offer.id !== offerId));
      setSelectedOffers(prev => prev.filter((id: string) => id !== offerId));
  ...
  const handleEditOffer = (offerId: string) => {
    alert(`Edit offer with ID: ${offerId}`);
  };

  const totalOffers = offers.length;
  const isAllSelected = selectedOffers.length === totalOffers && totalOffers > 0;
              ...
              <table className="w-full min-w-[600px]">
                ...
                <tbody className={`${isDarkMode ? 'bg-gray-800 divide-gray-700' : 'bg-white divide-gray-200'} divide-y`}>
                  {offers.length === 0 ? (
                    ...
                    offers.map((offer) => (
                      <tr key={offer.id} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                        ...
                        <td className={`px-3 py-2 text-xs ${isDarkMode ? 'text-gray-300' : 'text-gray-900'} font-medium`}>{offer.id}</td>
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-2">
                            <img
                              src={offer.publisherAvatar || '/default-avatar.svg'}
                              alt={offer.publisher}
                            ...
                            />
                            <span className={`text-xs ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{offer.publisher}</span>
                          </div>
                        </td>
                        <td className={`px-3 py-2 text-xs ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{offer.description}</td>
                        <td className="px-3 py-2">
                          <a
                        ...
                        <td className={`px-3 py-2 text-xs ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{offer.posted}</td>
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-2">
                            <button
