...
const GroupsSubCategories = () => {
  ...
  const fetchSubCategories = async () => {
    try {
      ...
      let response = await fetch(`${API_URL}/api/categories/groups-sub`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (!response.ok) {
        response = await fetch(`${API_URL}/api/categories/groups/subcategories`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
  ...
  const handleUpdateSubCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Try different possible API endpoints
      let response = await fetch(`${API_URL}/api/categories/groups-sub/${editingSubCategory._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      // If that fails, try alternative endpoint
      if (!response.ok) {
        response = await fetch(`${API_URL}/api/categories/groups/subcategories/${editingSubCategory._id}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleDeleteSubCategory = async (subCategoryId: string) => {
    ...
    try {
      ...
      // Try different possible API endpoints
      let response = await fetch(`${API_URL}/api/categories/groups-sub/${subCategoryId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      // If that fails, try alternative endpoint
      if (!response.ok) {
        response = await fetch(`${API_URL}/api/categories/groups/subcategories/${subCategoryId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      const deletePromises = Array.from(selectedIds).map(id => {
        // Try different possible API endpoints
        let response = fetch(`${API_URL}/api/categories/groups-sub/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        return response.catch(() => 
          fetch(`${API_URL}/api/categories/groups/subcategories/${id}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex mb-4" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-2 text-sm">
            ...
            <li>
              <span className={`${isDarkMode ? 'text-red-400' : 'text-red-500'} font-medium`}>Manage Groups Sub Categories</span>
            </li>
          </ol>
        </nav>
...
export default GroupsSubCategories;
