...
const ManageReplies = () => {
  ...
  const fetchReplies = async () => {
    try {
      ...
      // Try to fetch replies from API
      try {
        const endpoint = filter === 'All' 
          ? `${API_URL}/api/forums/replies`
          : `${API_URL}/api/forums/replies?status=${filter}`;
        ...
        if (response.ok) {
          ...
          // Sort replies
          const sorted = [...repliesList].sort((a: Reply, b: Reply) => {
            const aId = a._id || '';
            const bId = b._id || '';
  ...
  const filterReplies = () => {
    if (!searchQuery.trim()) {
      ...
      setFilteredReplies(replies.slice(startIndex, endIndex));
      return;
    }

    const filtered = replies.filter((reply) => {
      const title = (reply.title || reply.subject || '').toLowerCase();
      const description = (reply.description || reply.content || '').toLowerCase();
    ...
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    setFilteredReplies(filtered.slice(startIndex, endIndex));
  ...
  const handleSelectAll = (checked: boolean) => {
    const paginatedReplies = getPaginatedReplies();
    if (checked) {
      setSelectedReplies(paginatedReplies.map(r => r._id));
    } else {
      setSelectedReplies([]);
    }
  };
  ...
  const handleSelectReply = (replyId: string, checked: boolean) => {
    if (checked) {
      setSelectedReplies([...selectedReplies, replyId]);
    } else {
      setSelectedReplies(selectedReplies.filter(id => id !== replyId));
    }
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedReplies.length} reply/replies?`)) {
      return;
    }
    ...
    try {
      ...
      const deletePromises = selectedReplies.map(replyId =>
        fetch(`${API_URL}/api/forums/replies/${replyId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
      ...
      setSelectedReplies([]);
      await fetchReplies();
      alert('Replies deleted successfully!');
    } catch (error) {
      console.error('Error deleting replies:', error);
      alert('Failed to delete replies. Please try again.');
    } finally {
      setDeleting(false);
    }
  };

  const getAuthorName = (reply: Reply) => {
    if (reply.author?.name) return reply.author.name;
    if (reply.author?.username) return reply.author.username;
    return 'Unknown';
  };

  const getSubject = (reply: Reply) => {
    if (reply.subject) return reply.subject;
    if (reply.title) return reply.title;
    if (reply.content) {
      // Truncate content if it's too long
      return reply.content.length > 50 ? reply.content.substring(0, 50) + '...' : reply.content;
    }
    return '-';
  };
  ...
  const paginatedReplies = getPaginatedReplies();
  const showingCount = paginatedReplies.length;
  const totalCount = searchQuery.trim() 
    ? replies.filter(r => {
        const title = (r.title || r.subject || '').toLowerCase();
        const desc = (r.description || r.content || '').toLowerCase();
        const query = searchQuery.toLowerCase();
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex mb-4" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-2 text-sm">
            ...
            <li>
              <span className={`${isDarkMode ? 'text-red-400' : 'text-red-500'} font-medium`}>Manage Replies</span>
            </li>
          </ol>
        </nav>
              ...
              <thead className={`${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} border-b ${isDarkMode ? 'border-gray-600' : 'border-gray-200'}`}>
                <tr>
                  <th className="px-4 py-3 text-left">
                    <input
                      type="checkbox"
                      checked={selectedReplies.length === paginatedReplies.length && paginatedReplies.length > 0}
              ...
              <tbody className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center">
                      <div className="flex items-center justify-center">
                        ...
                        <span className={`ml-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading replies...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  paginatedReplies.map((reply) => (
                    <tr key={reply._id} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                      <td className="px-4 py-3">
                        <input
                          type="checkbox"
                          checked={selectedReplies.includes(reply._id)}
                          onChange={(e) => handleSelectReply(reply._id, e.target.checked)}
                          className={`rounded ${isDarkMode ? 'border-gray-600' : 'border-gray-300'} text-blue-600 focus:ring-blue-500`}
                        />
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {reply._id.substring(0, 8)}...
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {getAuthorName(reply)}
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        <div className="max-w-xs truncate" title={getSubject(reply)}>
                          {getSubject(reply)}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <button
                          onClick={async () => {
                            if (confirm('Are you sure you want to delete this reply?')) {
                              try {
                                const token = localStorage.getItem('token');
                                await fetch(`${API_URL}/api/forums/replies/${reply._id}`, {
                                  method: 'DELETE',
                                  headers: {
                                    'Authorization': `Bearer ${token}`,
                                    'Content-Type': 'application/json'
          ...
          <div className={`flex items-center justify-between border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} pt-4`}>
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                disabled={selectedReplies.length === 0 || deleting}
