...
const ManageForums = () => {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const forumsList = Array.isArray(data) ? data : (data.forums || []);
          
          // Sort forums
          const sorted = [...forumsList].sort((a: Forum, b: Forum) => {
            const aId = a._id || '';
            const bId = b._id || '';
        ...
        console.log('Forums endpoint not available');
        setForums([]);
        setTotalForums(0);
        setTotalPages(1);
      }
  ...
  const filterForums = () => {
    if (!searchQuery.trim()) {
      ...
      setFilteredForums(forums.slice(startIndex, endIndex));
      return;
    }
    ...
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    setFilteredForums(filtered.slice(startIndex, endIndex));
  ...
  const handleSelectAll = (checked: boolean) => {
    const paginatedForums = getPaginatedForums();
    if (checked) {
      setSelectedForums(paginatedForums.map(f => f._id));
    } else {
      setSelectedForums([]);
    }
  };

  const handleSelectForum = (forumId: string, checked: boolean) => {
    if (checked) {
      setSelectedForums([...selectedForums, forumId]);
    } else {
      setSelectedForums(selectedForums.filter(id => id !== forumId));
    }
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedForums.length} forum(s)?`)) {
      return;
    }
    ...
    try {
      ...
      const deletePromises = selectedForums.map(forumId =>
        fetch(`${API_URL}/api/forums/${forumId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
      ...
      setSelectedForums([]);
      await fetchForums();
      alert('Forums deleted successfully!');
    } catch (error) {
      console.error('Error deleting forums:', error);
      alert('Failed to delete forums. Please try again.');
    } finally {
      setDeleting(false);
    }
  };

  const getPaginatedForums = () => {
    if (searchQuery.trim()) {
      return filteredForums;
    }
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    return forums.slice(startIndex, endIndex);
  };

  const paginatedForums = getPaginatedForums();
  const showingCount = paginatedForums.length;
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex mb-4" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-2 text-sm">
            ...
            <li>
              <span className={`${isDarkMode ? 'text-red-400' : 'text-red-500'} font-medium`}>Manage Forums</span>
            </li>
          </ol>
        </nav>
              ...
              <thead className={`${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} border-b ${isDarkMode ? 'border-gray-600' : 'border-gray-200'}`}>
                <tr>
                  <th className="px-4 py-3 text-left">
                    <input
                      type="checkbox"
                      checked={selectedForums.length === paginatedForums.length && paginatedForums.length > 0}
              ...
              <tbody className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center">
                      <div className="flex items-center justify-center">
                        ...
                        <span className={`ml-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading forums...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  paginatedForums.map((forum) => (
                    <tr key={forum._id} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                      <td className="px-4 py-3">
                        <input
                          type="checkbox"
                          checked={selectedForums.includes(forum._id)}
                          onChange={(e) => handleSelectForum(forum._id, e.target.checked)}
                          className={`rounded ${isDarkMode ? 'border-gray-600' : 'border-gray-300'} text-blue-600 focus:ring-blue-500`}
                        />
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {forum._id.substring(0, 8)}...
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {forum.title || '-'}
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        <div className="max-w-xs truncate" title={forum.description || ''}>
                          {forum.description || '-'}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <button
                          onClick={async () => {
                            if (confirm('Are you sure you want to delete this forum?')) {
                              try {
                                const token = localStorage.getItem('token');
                                await fetch(`${API_URL}/api/forums/${forum._id}`, {
                                  method: 'DELETE',
                                  headers: {
                                    'Authorization': `Bearer ${token}`,
                                    'Content-Type': 'application/json'
          ...
          <div className={`flex items-center justify-between border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} pt-4`}>
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                disabled={selectedForums.length === 0 || deleting}
