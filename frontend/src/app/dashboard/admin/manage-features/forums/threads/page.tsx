...
const ManageThreads = () => {
  ...
  const fetchThreads = async () => {
    try {
      ...
      // Try to fetch threads from API
      try {
        const endpoint = filter === 'All' 
          ? `${API_URL}/api/forums/threads`
          : `${API_URL}/api/forums/threads?status=${filter}`;
        ...
        if (response.ok) {
          ...
          const threadsList = Array.isArray(data) ? data : (data.threads || []);
          
          // Sort threads
          const sorted = [...threadsList].sort((a: Thread, b: Thread) => {
            const aId = a._id || '';
            const bId = b._id || '';
        ...
        console.log('Threads endpoint not available');
        setThreads([]);
        setTotalThreads(0);
        setTotalPages(1);
      }
  ...
  const filterThreads = () => {
    if (!searchQuery.trim()) {
      ...
      setFilteredThreads(threads.slice(startIndex, endIndex));
      return;
    }

    const filtered = threads.filter((thread) => {
      const title = (thread.title || thread.headline || '').toLowerCase();
    ...
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    setFilteredThreads(filtered.slice(startIndex, endIndex));
  ...
  const handleSelectAll = (checked: boolean) => {
    const paginatedThreads = getPaginatedThreads();
    if (checked) {
      setSelectedThreads(paginatedThreads.map(t => t._id));
    } else {
      setSelectedThreads([]);
    }
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedThreads.length} thread(s)?`)) {
      return;
    }
    ...
    try {
      ...
      const deletePromises = selectedThreads.map(threadId =>
        fetch(`${API_URL}/api/forums/threads/${threadId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const getAuthorName = (thread: Thread) => {
    if (thread.author?.name) return thread.author.name;
    if (thread.author?.username) return thread.author.username;
    return 'Unknown';
  };

  const getHeadline = (thread: Thread) => {
    if (thread.headline) return thread.headline;
    if (thread.title) return thread.title;
    return '-';
  };

  const getPaginatedThreads = () => {
    if (searchQuery.trim()) {
  ...
  const totalCount = searchQuery.trim() 
    ? threads.filter(t => {
        const title = (t.title || t.headline || '').toLowerCase();
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex mb-4" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-2 text-sm">
            ...
            <li>
              <span className={`${isDarkMode ? 'text-red-400' : 'text-red-500'} font-medium`}>Manage Threads</span>
            </li>
          </ol>
        </nav>
              ...
              <thead className={`${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} border-b ${isDarkMode ? 'border-gray-600' : 'border-gray-200'}`}>
                <tr>
                  <th className="px-4 py-3 text-left">
                    <input
                      type="checkbox"
                      checked={selectedThreads.length === paginatedThreads.length && paginatedThreads.length > 0}
              ...
              <tbody className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center">
                      <div className="flex items-center justify-center">
                        ...
                        <span className={`ml-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading threads...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  paginatedThreads.map((thread) => (
                    <tr key={thread._id} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                      <td className="px-4 py-3">
                        <input
                          type="checkbox"
                          checked={selectedThreads.includes(thread._id)}
                          onChange={(e) => handleSelectThread(thread._id, e.target.checked)}
                          className={`rounded ${isDarkMode ? 'border-gray-600' : 'border-gray-300'} text-blue-600 focus:ring-blue-500`}
                        />
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {thread._id.substring(0, 8)}...
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        ...
                        <button
                          onClick={async () => {
                            if (confirm('Are you sure you want to delete this thread?')) {
                              try {
                                const token = localStorage.getItem('token');
                                await fetch(`${API_URL}/api/forums/threads/${thread._id}`, {
                                  method: 'DELETE',
                                  headers: {
                                    'Authorization': `Bearer ${token}`,
                                    'Content-Type': 'application/json'
          ...
          <div className={`flex items-center justify-between border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} pt-4`}>
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                disabled={selectedThreads.length === 0 || deleting}
