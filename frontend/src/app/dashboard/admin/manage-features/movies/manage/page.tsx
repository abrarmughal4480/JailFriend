...
const ManageMovies = () => {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Sort movies
          const sorted = [...moviesList].sort((a: Movie, b: Movie) => {
            const aId = a._id || '';
            const bId = b._id || '';
        ...
        console.log('Movies endpoint not available');
        setMovies([]);
        setTotalMovies(0);
        setTotalPages(1);
      }
  ...
  const filterMovies = () => {
    ...
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    setFilteredMovies(filtered.slice(startIndex, endIndex));
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    if (checked) {
      setSelectedMovies(paginatedMovies.map(m => m._id));
    } else {
      setSelectedMovies([]);
    }
  };
  ...
  const getMovieName = (movie: Movie) => {
    return movie.title || movie.name || 'Untitled Movie';
  };

  const getReleaseYear = (movie: Movie) => {
    return movie.releaseYear ? movie.releaseYear.toString() : '-';
  };
  ...
  const paginatedMovies = getPaginatedMovies();
  const showingCount = paginatedMovies.length;
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex mb-4" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-2 text-sm">
            ...
            <li>
              <span className={`${isDarkMode ? 'text-red-400' : 'text-red-500'} font-medium`}>Manage Movies</span>
            </li>
          </ol>
        </nav>
              ...
              <tbody className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center">
                      <div className="flex items-center justify-center">
                        ...
                        <span className={`ml-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading movies...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  paginatedMovies.map((movie) => (
                    <tr key={movie._id} className={isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                      <td className="px-4 py-3">
                        <input
                          type="checkbox"
                          checked={selectedMovies.includes(movie._id)}
                          onChange={(e) => handleSelectMovie(movie._id, e.target.checked)}
                          className={`rounded ${isDarkMode ? 'border-gray-600' : 'border-gray-300'} text-blue-600 focus:ring-blue-500`}
                        />
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {movie._id.substring(0, 8)}...
                      </td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                        {getMovieName(movie)}
                      </td>
                        ...
                        <button
                          onClick={async () => {
                            if (confirm('Are you sure you want to delete this movie?')) {
                              try {
                                const token = localStorage.getItem('token');
                                await fetch(`${API_URL}/api/movies/${movie._id}`, {
                                  method: 'DELETE',
                                  headers: {
                                    'Authorization': `Bearer ${token}`,
                                    'Content-Type': 'application/json'
          ...
          <div className={`flex items-center justify-between border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} pt-4`}>
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                disabled={selectedMovies.length === 0 || deleting}
