...
const CustomUsersFields = () => {
  ...
  const fetchFields = async () => {
    try {
      ...
      let response = await fetch(`${API_URL}/api/custom-fields/users`, {
        headers: {
      ...
      if (!response.ok) {
        response = await fetch(`${API_URL}/api/admin/custom-fields/users`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      const deletePromises = Array.from(selectedIds).map(id => {
        // Try different possible API endpoints
        let response = fetch(`${API_URL}/api/custom-fields/users/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        return response.catch(() => 
          fetch(`${API_URL}/api/admin/custom-fields/users/${id}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
  ...
  const handleDeleteField = async (fieldId: string) => {
    ...
    try {
      ...
      // Try different possible API endpoints
      let response = await fetch(`${API_URL}/api/custom-fields/users/${fieldId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      // If that fails, try alternative endpoint
      if (!response.ok) {
        response = await fetch(`${API_URL}/api/admin/custom-fields/users/${fieldId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex mb-4" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-2 text-sm">
            ...
            <li>
              <span className={`${isDarkMode ? 'text-red-400' : 'text-red-500'} font-medium`}>Manage Custom Profile Fields</span>
            </li>
          </ol>
        </nav>
