...
exports.getMyFollowers = async (req, res) => {
  try {
    ...
    const user = await User.findById(currentUserId)
      .populate('followers', 'name avatar username bio isOnline lastSeen isVerified');

    if (!user) {
      return res.status(404).json({ error: 'User not found' });
    }

    // Format the data for the messages
    const followersList = (user.followers || []).map(followerUser => ({
      _id: followerUser._id,
      name: followerUser.name || followerUser.fullName || 'User',
      username: followerUser.username || `@${followerUser._id.toString().slice(-8)}`,
      avatar: followerUser.avatar || '/avatars/1.png.png',
      isOnline: followerUser.isOnline || false,
      isVerified: followerUser.isVerified || false,
      bio: followerUser.bio || '',
      lastSeen: followerUser.lastSeen || ''
    }));
