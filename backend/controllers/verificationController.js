...
exports.submitVerification = async (req, res) => {
  try {
    ...
    if (req.files.passportDocument) {
      const passportFile = req.files.passportDocument[0];
      console.log('📸 Processing passport document:', passportFile.originalname);
      if (isCloudinaryConfigured) {
        passportDocumentUrl = passportFile.path;
      } else {
        passportDocumentUrl = `/uploads/${passportFile.filename}`;
      }
    }

    if (req.files.personalPicture) {
      const pictureFile = req.files.personalPicture[0];
      console.log('📸 Processing personal picture:', pictureFile.originalname);
      if (isCloudinaryConfigured) {
        personalPictureUrl = pictureFile.path;
      } else {
        personalPictureUrl = `/uploads/${pictureFile.filename}`;
      }
    }
